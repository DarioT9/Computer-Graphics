...
// Maximum number of tree instances supported in the scene
const int NTREE = 64;

// Uniform Buffer Object containing transformation matrices for each tree instance
// Each instance (tree) has its own set of matrices
layout(set = 1, binding = 0) uniform UniformBufferObject {
	mat4 mvpMat[NTREE]; // Model-View-Projection (MVP) matrix for each instance
	mat4 mMat[NTREE];   // Model matrix (transforms from local to world space)
	mat4 nMat[NTREE];   // Normal matrix (for correct normal transformations)
} ubo;
